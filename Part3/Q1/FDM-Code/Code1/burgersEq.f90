...
PROGRAM MAIN
    ...
    do it = 1, nsteps
        ...
        if ((mod(it, Ntime) .eq. 0) .or. (it .eq. 1)) then
            ...
            ! if ((mod(it, 1000*Ntime) .eq. 0).or. (it .eq. 1)) then
        ...
        if ((abs(PP-epsilon_fft) .LT. 4e-3) .and. (.not.convergent_judge)) then
            if (t .GT. 1) then
                write(*,*)'This case is converged !!!'
                convergent_judge = .True.
                write(60, 104)int(Re), N
            end if
        elseif ((abs(PP-epsilon_fft) .GT. 4e-3) .and. it .eq. nsteps) then
