MODULE common_data
    ...
    real*8, parameter           :: Len=1.d0, u0=1.d0, nu=0.001d0, CFL=0.025d0, t_tilde=3.2d0
...
PROGRAM MAIN
    ...
    open(56, file='spatialAveraged.dat', status='replace', action='write')
    open(57, file='uSteadySolution.dat', status='replace', action='write')
    open(58, file='uLocalSteadySolution.dat', status='replace', action='write')
    ...
    do it = 1, nsteps
        ...
        if ((mod(it, Ntime) .eq. 0) .or. (it .eq. 1)) then
            ...
            if ((mod(it, 10*Ntime) .eq. 0).or. (it .eq. 1)) then
                ! do j = 4, N+3
                !     write(55, 100)t, x(j), u(j)
                ! end do
                write(56, 101)t, ke, epsilon, PP, (PP - epsilon), dkedt, etSAV
            end if
        end if
        ...
        if (it .eq. nsteps) then
            do j = 1, N+1
                write(58, 100)t, x_interface(j), u_interface(j)
            end do
        end if
